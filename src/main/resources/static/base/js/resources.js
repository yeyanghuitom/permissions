...
function getResourcesBindDetailById (str){
    $.ajax({
        type:'post',
            ...
            try{
                if(data.code == "000"){
                    if(data.resourcesList.length == 0){
                        ...
                        deleteResourcesBindAllById(str);
                    }else{
                        swal({
                                title: "资源信息被占用",
                                text: "删除当前资源信息被占用",
...
function deleteResourcesBindAllById(){
    $.ajax({
        type:'post',
        url:ajaxBaseURL+"/resources/deleteResourcesById",
        data:"RESOURCE_ID="+$table.rowData[$table.key],
        success:function(data){
            try{
                if(data.code=="000"){
...
$("#userPlus_submit").on("click",function(){
        ...
        function(){
            var headData=$table.headData,
                data="",all_data=$table.all_data,obj={};
            for(var k in all_data){
                let str=$("#userPlus_"+k).val();
                if(!regForInput(str) && k != 'URL' && k != "ACTION_URL" && k != "RESOURCE_ID"){
                    console.log(k);
                    console.log(str);
                    swal("违规填写数据","填写数据只能含有汉字、大小写字母、“_”或是数字","error");return"";
                }

                if( k == "RESOURCE_NAME" && str == "" ){
                    swal("请填写资源名","资源名字段不可为空","warning");
                    return "";
                }else if( k == "RESOURCE_ID"){//04-13 resourceID 屏蔽
                    var k1 = uuid();
                    data+= k+"="+ k1 +"&";
                    obj[k]=k1;
...
$("#setMsg_submit").on("click",function(){
    swal({
        ...
        var rowData=$table.rowData,headData=$table.setMsg_data,str="";
        for(var k in headData){
            let str=$("#setMsg_"+k).val();
            if( k == 'URL' || k == "ACTION_URL" ){
                rowData[k]=str;
            }else if(regForInput(str) && k != "RESOURCE_ID"){
                rowData[k]=str;
            } else if(k != "RESOURCE_ID"){
                console.log("修改方法打印");
                console.log(k);
                console.log(str);
                swal("修改信息，违规填写数据","填写数据只能含有汉字、大小写字母、“_”或是数字,资源路径与ACTION路径不提供校验","error");
                return""
            }
        }
        for(var k in rowData){
            let str1=$("#setMsg_"+k).val();

            if( k == "RESOURCE_NAME" && str1 == "" ){
                swal("请填写资源名","资源名字段不可为空","warning");
                return "";
            }else if(k=="CREATE_DATE"){
                str+=(k+"=now()&");
...
function clearChildrenPage(data){
    var id=data.id;var obj=$table[data.id+"_data"];
    for(var k in obj){
        if( k == "IS_ENT" ){
...
$().ready(function(){
    ...
    window.operateFormatter=function(value,row,USER_ID){
        return['<a rel="tooltip" title="查看信息" class="btn btn-simple btn-info btn-icon table-action getMsg" data-toggle="modal" data-target="#getMsg">','<i class="fa fa-search"></i>','</a> ',' <a rel="tooltip" title="修改信息" class="btn btn-simple btn-warning btn-icon table-action setMsg" data-toggle="modal" data-target="#setMsg">','<i class="fa fa-pencil"></i>','</a> ',' <a rel="tooltip" title="查看当前页面的绑定按键" class="btn btn-simple btn-warning btn-icon table-action resourcesBindButton">','<i class="fa fa-link"></i>','</a> ',' <a rel="tooltip" title="删除信息" class="btn btn-simple btn-danger btn-icon table-action remove">','<i class="fa fa-trash"></i>','</a> '].join('');
    };
    window.operateEvents={
        'click .getMsg':function(e,value,row,RESOURCE_ID){ clearChildrenPage({id:"getMsg"});$table.rowData=row;getOrsetShowMsg({id:"getMsg",rowData:row});$.ajax({type:'post',url:ajaxBaseURL+"/"+baseField+"/getOne"+baseField.slice(0,1).toUpperCase()+baseField.slice(1)+"ById",data:$table.key+"="+row[$table.key],success:function(data){try{console.log(data);window.str=JSON.stringify(data);var objData=$table["getMsg_data"],value=data.value;for(var k in objData){$("#getMsg_data_"+objData[k]).val(value[k]);}}catch(error){console.log("【 bootstrapTable - 返回结果 - 报错 】");console.log(error);}},error:function(){console.log("【 bootstrapTable - 报错 】");console.log(error);}});
        },
        'click .setMsg':function(e,value,row,index){
            $("#setMsg_RESOURCE_ID").attr("disabled","disabled");
        ...
        'click .remove':function(e,value,row,RESOURCE_ID){
            console.log(row);
            $table.rowData=row;
            swal({title:"是否删除数据",text:"将删除此条数据的所有信息",type:"warning",confirmButtonText:"删除",cancelButtonText:"取消",showCancelButton:true,closeOnConfirm:false,showLoaderOnConfirm:true,},
                function(){
    ...
    function transferAllDataProcessing(head){
        ...
        $table.userPlus_data=userPlus_data;allDataProcessing({pageId:"getMsg_from",obj:getMsg_data});
        $table.getMsg_data=getMsg_data;allDataProcessing({pageId:"setMsg_from",obj:setMsg_data});
        ...
        $.ajax({
            ...
            success:function(data){
                try{
                    ...
                    window.$table=$table;window.$table.key=obj.key;
                }catch(error){
                    console.log("【 bootstrapTable - 返回结果 - 报错 】");
                    console.log(error);
                }
...
function resourcesBindButtonTable(){
    ...
    $("#createOneButtonMsg_submit").on("click",function(){
        var list=["SORT_CODE","RESOURCE_ID","BUTTON_NAME","REMARK","BUTTON_URL"],
            str="";
        for(var i=0;i<list.length;i++){str+="&"+list[i]+"="+$("#createOneButtonMsg_"+list[i]).val();}
        swal({
            title:"确认创建当前按钮信息",
            text:"将创建当前按钮信息",
            type:"warning",
            ...
            $.ajax({
                type:'post',
                url:ajaxBaseURL+"/button/insertButton",
                data:"BUTTON_ID="+BUTTON_ID+"&"+str,
                success:function(data){
                    try{
                        if(data.code=="000"){
                            swal("创建按钮成功！","","success");
                            $("#createOneButtonMsg").modal('hide');
                            var inputObj={BUTTON_ID:BUTTON_ID};
                            for(var i=0;i<list.length;i++){
    ...
    $("#setOneButtonMsg_submit").on("click",function(){
        var id=$table1.rowId;
        swal({
            title:"是否修改 BUTTON 数据",
            text:"将删除此条 BUTTON 数据的所有信息",
            type:"warning",
            ...
            $.ajax({
                type:'post',
                url:ajaxBaseURL+"/button/updateButtonById",
                data:"BUTTON_ID="+id+str,
                success:function(data){
                    try{
                        if(data.code=="000"){
                            swal("数据修改成功！","","success");
                            $table1.bootstrapTable('remove',{field:"BUTTON_ID",values:[$table1.rowId]});
    ...
    window.operateFormatterOfButtonTable=function(value,row,USER_ID){
            return ['<a rel="tooltip" title="修改 BUTTON 信息" class="btn btn-simple btn-warning btn-icon table-action setOneButtonMsg" data-toggle="modal" data-target="#setOneButtonMsg">','<i class="fa fa-pencil"></i>','</a> ','<a rel="tooltip" title="删除 BUTTON 信息" class="btn btn-simple btn-danger btn-icon table-action removeOneButtonMsg">','<i class="fa fa-trash"></i>','</a> '].join('');
    };
    window.operateEventsOfButtonTable={
            'click .getMsg':function(e,value,row,RESOURCE_ID){
                clearChildrenPage({id:"getMsg"});
                $table.rowData=row;getOrsetShowMsg({id:"getMsg",rowData:row});
                $.ajax({ type:'post',url:ajaxBaseURL+"/"+baseField+"/getOne"+baseField.slice(0,1).toUpperCase()+baseField.slice(1)+"ById",data:$table.key+"="+row[$table.key],success:function(data){try{console.log(data);window.str=JSON.stringify(data);var objData=$table["getMsg_data"],value=data.value;for(var k in objData){$("#getMsg_data_"+objData[k]).val(value[k]);}}catch(error){console.log("【 bootstrapTable - 返回结果 - 报错 】");console.log(error);}},error:function(error){console.log("【 bootstrapTable - 报错 】");console.log(error);}});
                },
            'click .setMsg':function(e,value,row,index){
                clearChildrenPage({id:"setMsg"});
                $table.rowData=row;getOrsetShowMsg({id:"setMsg",rowData:row});
                },
            'click .remove':function(e,value,row,RESOURCE_ID){
                console.log(row);
                $table.rowData=row;swal({title:"是否删除数据",text:"将删除此条数据的所有信息",type:"warning",confirmButtonText:"删除",cancelButtonText:"取消",showCancelButton:true,closeOnConfirm:false,showLoaderOnConfirm:true,},function(){$table.bootstrapTable('remove',{field:'id',values:[row.id]});});
                },
            'click .removeOneButtonMsg':function(e,value,row,RESOURCE_ID){
                $table1.key="BUTTON_ID";$table1.rowData=row;swal({title:"是否删除按钮信息",text:"将删除此条按钮的所有信息",type:"warning",confirmButtonText:"删除",cancelButtonText:"取消",showCancelButton:true,closeOnConfirm:false,showLoaderOnConfirm:true,},function(){$.ajax({type:'post',url:ajaxBaseURL+"/button/deleteButtonById",data:$table1.key+"="+row[$table1.key],success:function(data){try{if(data.code=="000"){swal("数据成功删除！","","success");$table1.bootstrapTable('remove',{field:$table1.key,values:[row[$table1.key]]});console.log(JSON.stringify(data));}else{swal("数据删除失败！",/*JSON.stringify(data)*/"","error");}}catch(error){console.log("【 bootstrapTable - 返回结果 - 报错 】");console.log(error);}},error:function(error){console.log("【 bootstrapTable - 报错 】");console.log(error);}});});
                },
            'click .setOneButtonMsg':function(e,value,row){
                window["$table1"].rowId=row.BUTTON_ID;console.log("row.BUTTON_ID");console.log(row.BUTTON_ID);
                $.ajax({
                    type:'post',url:ajaxBaseURL+"/button/getOneButtonById",data:"BUTTON_ID="+row.BUTTON_ID,success:function(data){try{if(data.code=="000"){swal("获取数据成功！",/*JSON.stringify(data)*/"","success");var obj=data.value;for(var key in obj){$("#setOneButtonMsg_"+key).val(obj[key]);}}else{swal("获取数据失败！",/*JSON.stringify(data)*/"","error");}}catch(error){console.log("【 bootstrapTable - 返回结果 - 报错 】");console.log(error);}},error:function(error){console.log("【 bootstrapTable - 报错 】");console.log(error);}
                });
                },
        };
