...
$(function () {
    ...
    //经度校验
    jQuery.validator.addMethod("isJD", function (value, element) {
        var reg = /^(\-|\+)?(((\d|[1-9]\d|1[0-7]\d|0{1,3})\.\d{0,6})|(\d|[1-9]\d|1[0-7]\d|0{1,3})|180\.0{0,6}|180)$/;
        return this.optional(element) || (reg.test(value));
    }, "经度整数部分为0-180,小数部分为0到6位!");

    //纬度校验
    jQuery.validator.addMethod("isWD", function (value, element) {
        var reg = /^(\-|\+)?([0-8]?\d{1}\.\d{0,6}|90\.0{0,6}|[0-8]?\d{1}|90)$/;
        return this.optional(element) || (reg.test(value));
    }, "纬度整数部分为0-90,小数部分为0到6位!");

    //纬度校验
    jQuery.validator.addMethod("isCjzhm", function (value, element) {
        var card = value.substring(0, 18);
        var cjhm = value.substring(18);
        var reg = /^[1-7][1-4]$/;
        return this.optional(element) || (isIdCardNo(card) && reg.test(cjhm));
    }, "请输入正确的残疾证号码");
});
//身份证号码的验证规则
